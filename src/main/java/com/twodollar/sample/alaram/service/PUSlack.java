...
@Service
public class PUSlack
{
	private static final Logger logger = LoggerFactory.getLogger(PUSlack.class);
...
//	private DUMessengerHistory dumh;
	...
	public HashMap<String,Object> chatPostMessage(HashMap<String,Object> requestMap, HashMap<String,Object> configMap){
		...
		if(configMap!= null) {
			...
			String snd_msg = requestMap.get("snd_msg").toString();
			if (snd_msg.length() > 500) {
				snd_msg = snd_msg.substring(0, 500);
			} 
			
			//메시지 전송
			Map<String,Object> queryParam = (Map<String, Object>) configMap.clone();
			queryParam.put("text", snd_msg);
